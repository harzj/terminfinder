...
import { clampPlanningMonths, getPlanningDays } from '@/lib/planningWindow'
...
export default function GruppenUebersicht({ members, availabilities, events, startDate, endDate, blockedDates, currentUserId }: Props) {
  // Zeitraum aus dem Server: aktueller Montag bis zum Gruppen-Maximum.
  ...
  const dayIndexByDate = new Map(allDays.map((d, idx) => [d, idx]))
  ...
  return (
    <div className="-mx-4">
      <div className="flex">
        ...
        <div className="flex-1 overflow-x-auto pr-4">
          ...
          {members.map((member: any) => {
            ...
            const memberPlanningMonths = clampPlanningMonths(member.profiles?.availability_planning_months)
            const memberPlanningDays = getPlanningDays(memberPlanningMonths)
            return (
              <div key={member.user_id} className="flex items-center mb-1 h-7">
                {days.map((d) => {
                  ...
                  // Über den persönlichen Planungshorizont hinaus als dunkleres Grau darstellen.
                  const dayIndex = dayIndexByDate.get(d) ?? 0
                  const isOutOfMemberRange = dayIndex >= memberPlanningDays
                  ...
                  return (
                    <div
                      ...
                      title={
                        isOutOfMemberRange
                          ? 'außerhalb persönlichem Planungshorizont'
                          : isBlockedOtherGroup
                            ? 'Termin in anderer Gruppe'
                            : avail
                              ? `${avail.status}${avail.from_time ? ` ${avail.from_time.slice(0, 5)}–${avail.until_time?.slice(0, 5) ?? ''}` : ''}`
                              : 'keine Angabe'
                      }
                    >
      ...
      <div className="flex flex-wrap gap-3 mt-4 px-4 text-xs text-muted-foreground">
        ...
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded bg-muted-foreground/30 inline-block" /> Außerhalb persönlichem Zeitraum
        </span>
      </div>
    </div>
  )
